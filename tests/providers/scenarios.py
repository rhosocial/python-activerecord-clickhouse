...
"""ClickHouse backend test scenario configuration mapping table"""
...
# Scenario name -> configuration dictionary mapping table (ClickHouse only)
SCENARIO_MAP: Dict[str, Dict[str, Any]] = {}


def register_scenario(name: str, config: Dict[str, Any]):
    """Register ClickHouse test scenario"""
    SCENARIO_MAP[name] = config


def get_scenario_raw(name: str) -> Tuple[Type[ClickHouseBackend], ClickHouseConnectionConfig]:
    ...
    config = ClickHouseConnectionConfig(**SCENARIO_MAP[name])
...
def _load_scenarios_from_config():
    ...
    if env_config_path and os.path.exists(env_config_path):
        ...
        config_path = os.path.join(os.path.dirname(__file__), "..", "config", "clickhouse_scenarios.yaml")
        if not os.path.exists(config_path):
            raise FileNotFoundError(
                "No ClickHouse scenarios configuration file found. "
                "Either set CLICKHOUSE_SCENARIOS_CONFIG_PATH environment variable to point to a valid YAML file "
                "or place clickhouse_scenarios.yaml in the tests/config directory."
            )
        print(f"Loading ClickHouse scenarios from default path: {config_path}")
...
def _apply_scenario_filter():
    ...
    local override (CLICKHOUSE_ACTIVE_SCENARIOS). It contains comma-separated full
    scenario names (e.g., "clickhouse_97,clickhouse_80").
    """
