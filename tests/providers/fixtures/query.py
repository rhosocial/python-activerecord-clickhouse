...
def create_users_table(dialect, table_name: str = "users") -> CreateTableExpression:
    return CreateTableExpression(
        ...
        columns=[
            ...
            ColumnDefinition(dialect, "balance", DoubleType(),
                constraints=[ColumnConstraint(dialect, ColumnConstraintType.NOT_NULL),
...
def create_posts_table(dialect, table_name: str = "posts") -> CreateTableExpression:
    return CreateTableExpression(
        ...
        columns=[
            ColumnDefinition(dialect, "id", BigIntType(),
                constraints=[ColumnConstraint(dialect, ColumnConstraintType.PRIMARY_KEY)]),
            ColumnDefinition(dialect, "user_id", BigIntType(),
                constraints=[ColumnConstraint(dialect, ColumnConstraintType.NOT_NULL)]),
...
def create_comments_table(dialect, table_name: str = "comments") -> CreateTableExpression:
    return CreateTableExpression(
        ...
        columns=[
            ...
            ColumnDefinition(dialect, "user_id", BigIntType(),
                constraints=[ColumnConstraint(dialect, ColumnConstraintType.NOT_NULL)]),
            ColumnDefinition(dialect, "post_id", BigIntType(),
                constraints=[ColumnConstraint(dialect, ColumnConstraintType.NOT_NULL)]),
            ColumnDefinition(dialect, "content", TextType()),
            ColumnDefinition(dialect, "is_hidden", BooleanType(),
...
def create_orders_table(dialect, table_name: str = "orders") -> CreateTableExpression:
    return CreateTableExpression(
        ...
        columns=[
            ...
            ColumnDefinition(dialect, "user_id", BigIntType(),
                constraints=[ColumnConstraint(dialect, ColumnConstraintType.NOT_NULL)]),
            ColumnDefinition(dialect, "order_number", VarCharType(length=255),
                constraints=[ColumnConstraint(dialect, ColumnConstraintType.NOT_NULL)]),
...
def create_order_items_table(dialect, table_name: str = "order_items") -> CreateTableExpression:
    return CreateTableExpression(
        ...
        columns=[
            ColumnDefinition(dialect, "id", BigIntType(),
                constraints=[ColumnConstraint(dialect, ColumnConstraintType.PRIMARY_KEY)]),
            ColumnDefinition(dialect, "order_id", BigIntType(),
                constraints=[ColumnConstraint(dialect, ColumnConstraintType.NOT_NULL)]),
            ColumnDefinition(dialect, "product_name", VarCharType(length=255),
                constraints=[ColumnConstraint(dialect, ColumnConstraintType.NOT_NULL)]),
...
def create_profiles_table(dialect, table_name: str = "profiles") -> CreateTableExpression:
    return CreateTableExpression(
        ...
        columns=[
            ...
            ColumnDefinition(dialect, "avatar_url", VarCharType(length=512)),
            ColumnDefinition(dialect, "created_at", ClickHouseDateTime64Type(precision=6)),
            ColumnDefinition(dialect, "updated_at", ClickHouseDateTime64Type(precision=6)),
        ],
        storage_options=dict(_DEFAULT_STORAGE_OPTIONS),
...
def create_json_users_table(dialect, table_name: str = "json_users") -> CreateTableExpression:
    return CreateTableExpression(
        ...
        columns=[
            ...
            ColumnDefinition(dialect, "created_at", ClickHouseDateTime64Type(precision=6)),
            ColumnDefinition(dialect, "updated_at", ClickHouseDateTime64Type(precision=6)),
            ColumnDefinition(dialect, "settings", JsonType()),
            ColumnDefinition(dialect, "tags", JsonType()),
            ColumnDefinition(dialect, "profile", JsonType()),
            ColumnDefinition(dialect, "roles", JsonType()),
            ColumnDefinition(dialect, "scores", JsonType()),
            ColumnDefinition(dialect, "subscription", JsonType()),
            ColumnDefinition(dialect, "preferences", JsonType()),
        ],
        storage_options=dict(_DEFAULT_STORAGE_OPTIONS),
    )
    ...
    return CreateTableExpression(
        ...
        columns=[
            ...
            ColumnDefinition(dialect, "name", VarCharType(length=255),
                constraints=[ColumnConstraint(dialect, ColumnConstraintType.NOT_NULL)]),
            ColumnDefinition(dialect, "parent_id", BigIntType(),
    ...
    return CreateTableExpression(
        ...
        columns=[
            ...
            ColumnDefinition(dialect, "status", VarCharType(length=50),
                constraints=[ColumnConstraint(dialect, ColumnConstraintType.NOT_NULL),
                             ColumnConstraint(dialect, ColumnConstraintType.DEFAULT, default_value="pending")]),
            ColumnDefinition(dialect, "priority", VarCharType(length=50),
                constraints=[ColumnConstraint(dialect, ColumnConstraintType.NOT_NULL),
                             ColumnConstraint(dialect, ColumnConstraintType.DEFAULT, default_value="medium")]),
            ColumnDefinition(dialect, "region", VarCharType(length=50),
                constraints=[ColumnConstraint(dialect, ColumnConstraintType.NOT_NULL),
                             ColumnConstraint(dialect, ColumnConstraintType.DEFAULT, default_value="default")]),
            ColumnDefinition(dialect, "category", VarCharType(length=255)),
            ColumnDefinition(dialect, "product", VarCharType(length=255)),
            ColumnDefinition(dialect, "department", VarCharType(length=255)),
            ColumnDefinition(dialect, "year", VarCharType(length=10)),
            ColumnDefinition(dialect, "quarter", VarCharType(length=10)),
            ColumnDefinition(dialect, "created_at", ClickHouseDateTime64Type(precision=6)),
            ColumnDefinition(dialect, "updated_at", ClickHouseDateTime64Type(precision=6)),
        ],
        indexes=[IndexDefinition(dialect, name="idx_user_id", columns=["user_id"]),
