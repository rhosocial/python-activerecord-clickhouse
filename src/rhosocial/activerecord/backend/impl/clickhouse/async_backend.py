...
"""
Asynchronous ClickHouse-specific implementation of the AsyncStorageBackend.

This module provides the concrete async implementation for interacting with ClickHouse databases,
...
from .mixins import ClickHouseBackendMixin, AsyncClickHouseConcurrencyMixin
...
class AsyncClickHouseBackend(
    ...
    """Asynchronous ClickHouse-specific backend implementation."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize async ClickHouse backend with connection configuration.
        ...
        self._transaction_manager = AsyncClickHouseTransactionManager(self, self.logger)

        # Register ClickHouse-specific type adapters (uses self._version)
        self._register_clickhouse_adapters()

        self.log(logging.INFO, "AsyncClickHouseBackend initialized")

    def _create_introspector(self) -> Any:
        """Create an AsyncClickHouseIntrospector backed by an AsyncIntrospectorExecutor."""
        from rhosocial.activerecord.backend.introspection.executor import AsyncIntrospectorExecutor
        from .introspection import AsyncClickHouseIntrospector

        return AsyncClickHouseIntrospector(self, AsyncIntrospectorExecutor(self))
    ...
    async def connect(self) -> None:
        """Establish async connection to ClickHouse database."""
        try:
            ...
            # Only include parameters that are supported by clickhouse-connector-python aio
            additional_params = [
                "auth_plugin",
                "init_command",
                "connect_timeout",
            ...
            self._connection = await clickhouse_async.connect(**conn_params)
    ...
    async def disconnect(self) -> None:
        """Close async connection to ClickHouse database."""
        if self._connection:
            ...
            try:
                ...
                # Python 3.8 + clickhouse-connector-python has a known issue where
                # closing a connection raises RuntimeError during cursor set iteration:
                # "Set changed size during iteration" in clickhouse/connector/aio/connection.py:672
                ...
                # See: https://bugs.clickhouse.com/?id=114095
                if "Set changed size during iteration" in str(e):
                    self.log(logging.WARNING, f"Python 3.8 clickhouse-connector cursor cleanup issue (ignored): {str(e)}")
                else:
    ...
    async def get_server_version(self) -> Tuple[int, int, int]:
        """Get ClickHouse server version asynchronously."""
    ...
    async def ping(self, reconnect: bool = True) -> bool:
        ...
        try:
            ...
            try:
                ...
                # May occur in ClickHouse 5.6 RST race condition or asyncio transport
                if reconnect:
                    await self.disconnect()
                    await self.connect()
                    return True
    ...
    async def _handle_auto_commit(self) -> None:
        """Handle auto commit based on ClickHouse connection and transaction state asynchronously.
    ...
    async def _handle_auto_commit_if_needed(self) -> None:
        """
        Handle auto-commit for ClickHouse asynchronously.
    ...
    async def executescript(self, sql_script: str) -> None:
        ...
        individually, which is compatible with aioclickhouse's cursor interface.
        ...
        try:
            ...
            # aioclickhouse does not support multi=True like clickhouse-connector-python.
