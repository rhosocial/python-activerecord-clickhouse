# src/rhosocial/activerecord/backend/impl/clickhouse/cli/info.py
"""info subcommand - Display ClickHouse environment information.
...
# Groups that are specific to ClickHouse dialect
DIALECT_SPECIFIC_GROUPS = {"ClickHouse-specific"}
...
SUPPORT_METHOD_ALL_ARGS: Dict[str, List[str]] = {
    ...
    # ClickHouseJSONFunctionSupport: JSON functions with version-specific support
    ...
    # ClickHouseSpatialSupport: spatial types
...
def _build_protocol_info(dialect, group_name: str, protocols: List[type], verbose: int) -> Dict[str, Dict[str, Any]]:
    ...
    Args:
        dialect: ClickHouseDialect instance to check against
...
def _display_info_rich(info: Dict, verbose: int, version_display: str, is_connected: bool = True):
    ...
    console.print("\n[bold cyan]ClickHouse Environment Information[/bold cyan]\n")
