# Table of Contents

* [rhosocial-activerecord ClickHouse Backend Documentation](README.md)

## Introduction

* [Overview](introduction/README.md)
* [Relationship with the core library](introduction/relationship.md)
* [Supported versions](introduction/supported_versions.md)
* [Capability boundaries & fail-fast](introduction/capability_boundaries.md)

## Installation

* [Installation guide](installation/installation.md)
* [Connection configuration](installation/configuration.md)

## Getting Started

* [Quick start](getting_started/quick_start.md)
* [First CRUD application](getting_started/first_crud.md)

## Modeling

* [Field type mapping](modeling/field_types.md)
* [Nullable & optional fields](modeling/nullable.md)

## Querying

* [JSON queries](querying/json.md)
* [Arrays, Maps, Tuples](querying/arrays_maps.md)

## DDL

* [DDL feature specs](ddl/ddl_spec.md)
* [Table engines & sorting key](ddl/table_engine.md)
* [Skip indexes](ddl/skip_indexes.md)

## Capabilities

* [Client-side snowflake IDs](capabilities/snowflake_ids.md)
* [Mutations (UPDATE/DELETE)](capabilities/mutations.md)
* [Unsupported features](capabilities/unsupported.md)

## Command Line

* [CLI usage](cli/README.md)

## Testing

* [Test configuration](testing/README.md)
