# 目录

* [rhosocial-activerecord ClickHouse 后端文档](README.md)

## 简介

* [概述](introduction/README.md)
* [与核心库的关系](introduction/relationship.md)
* [支持版本](introduction/supported_versions.md)
* [能力边界与快速失败](introduction/capability_boundaries.md)

## 安装

* [安装指南](installation/installation.md)
* [连接配置](installation/configuration.md)

## 快速入门

* [快速开始](getting_started/quick_start.md)
* [第一个 CRUD 应用](getting_started/first_crud.md)

## 模型定义

* [字段类型映射](modeling/field_types.md)
* [Nullable 与可选字段](modeling/nullable.md)

## 查询

* [JSON 查询](querying/json.md)
* [数组、Map、Tuple](querying/arrays_maps.md)

## DDL

* [DDL 特征 Spec](ddl/ddl_spec.md)
* [表引擎与排序键](ddl/table_engine.md)
* [跳数索引](ddl/skip_indexes.md)

## 能力特性

* [客户端雪花 ID](capabilities/snowflake_ids.md)
* [变更（UPDATE/DELETE）](capabilities/mutations.md)
* [不支持的功能](capabilities/unsupported.md)

## 命令行

* [CLI 用法](cli/README.md)

## 测试

* [测试配置](testing/README.md)
